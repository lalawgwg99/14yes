    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-black/40 backdrop-blur-sm fade-in-up" onClick={onClose}>
      <div className="bg-paper max-w-sm w-full p-8 border border-ink shadow-2xl relative torn-paper" onClick={e => e.stopPropagation()}>
        ...
        <div className="flex flex-col items-center text-center space-y-4">
          ...
          <div>
            <h2 className="text-3xl serif-tc font-bold text-ink">{displayName}</h2>
            <p className="text-xs uppercase tracking-[0.2em] text-bronze mt-2 font-bold">{agent.title}</p>
          </div>
          ...
          <div className="space-y-6 py-2 w-full text-left">
            ...
            <div>
              ...
              <div className="h-3 bg-gray-200 rounded-full relative">
                <div className="absolute top-1/2 -translate-y-1/2 w-4 h-4 bg-bronze rounded-full border-2 border-white shadow-sm" style={{ left: `calc(${agent.profile.system1Ranking}% - 8px)` }} />
              </div>
            </div>
          </div>
